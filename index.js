...
//  start returing data on page from localstroge
...
const getDataFromLocal = () => {
    userData.forEach((data, index) => {
        tableData.innerHTML += `
            <tr index='${index}'>
                <td>${index + 1}</td>
                <td><img src="${data.profilePic}" width="30"></td>
                <td>${data.id}</td>
                <td>${data.name}</td>
                <td>${data.l_name}</td>
                <td>${data.email}</td>
                ...
                <td>
                    ...
                    <button class="del-btn" style="background-color:#eb0b30"><i class="fa fa-trash"></i></button>
                </td>
            </tr>
        `;
    });
     ...
     for(i=0;i<allDelBtn.length;i++){
        allDelBtn[i].onclick = function(){
            ...
            swal({
              ...
              .then((willDelete) => {
                if (willDelete) {  userData.splice(id,1);
...
// start claer all data

var delAllBtn = document.querySelector("#del-all-btn");
var allDelBtn = document.querySelector("#del-all-box");

delAllBtn.addEventListener('click', () => {
    if (allDelBtn.checked) {
        swal({
            title: "Are you sure?",
            text: "Once deleted, you will not be able to recover this imaginary file!",
            icon: "warning",
